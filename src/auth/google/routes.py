...
def login():
    flow = Flow.from_client_config(
        ...
        ,scopes=current_app.config["GOOGLE_ALLOWED_SCOPES"]
    )
...
def logout():
    """
    Logs out the user by revoking the Google OAuth token and clearing the session.
    
    Steps:
    1. Retrieve the access token from session credentials
    2. Revoke the token with Google's revoke endpoint
    3. Clear all session data
    4. Redirect to home page or configured logout redirect
    """
    ...
    if access_token:
        try:
            ...
            # Log the error but continue with logout even if revocation fails
            current_app.logger.warning(f"Token revocation failed: {str(e)}")
...
@auth_bp.route('/logout-and-redirect', methods=['GET', 'POST'])
def logout_and_redirect():
    """
    Logs out the user and redirects to a specified URL.
    
    Query parameter:
    - redirect_url: The URL to redirect to after logout (defaults to home page)
    """
